...
//空间关注操作
function dofollow_ig(type,target,uid){
	var html = '';
	$('#follow_state').html( '<img src="'+ _THEME_+'/images/icon_waiting.gif" width="15">' );
	$.post( U('weibo/Operate/follow') ,{uid:uid,type:type},function(txt){
		if(txt=='12'){
			html = followState_ig('havefollow');
			followGroupSelectorBox(uid);
		}else if(txt=='13'){
			html = followState_ig('eachfollow');
			followGroupSelectorBox(uid);
		}else if(txt=='00'){
			ui.error('对方不允许你关注');
			html = followState_ig('unfollow',target,uid);
		}else{
			html = followState_ig();
		}
		$('#follow_state').html( html );
...
//列表关注操作
function dolistfollow_ig(type,target,uid){
	var html = '';
	var target=target;
	var uid=uid;
	$("#follow_list_"+uid).html( '<img src="'+ _THEME_+'/images/icon_waiting.gif" width="15">' );
	$.post( U('weibo/Operate/follow') ,{uid:uid,type:type},function(txt){
		if(txt=='12'){
			html = followState_ig('havefollow',target,uid);
			followGroupSelectorBox(uid);
		}else if(txt=='13'){
			html = followState_ig('eachfollow',target,uid);
			followGroupSelectorBox(uid);
		}else if(txt=='00'){
			ui.error('对方不允许你关注');
			html = followState_ig('unfollow',target,uid);
		}else{
			html = followState_ig('',target,uid);
		}
		$("#follow_list_"+uid).html( html );
	});
}
...
//关注状态
function followState_ig(type,target,uid){
	target = target || 'dofollow';
	uid    = uid    || _UID_;
	if(type=='havefollow'){
		html = '<span>已关注&nbsp;&nbsp;|&nbsp;&nbsp;</span><a href="javascript:void(0);" onclick="'+target+'(\'unflollow\',\''+target+'\','+uid+')">取消</a>';
	}else if(type=='eachfollow'){
		html = '<span>互相关注&nbsp;&nbsp;|&nbsp;&nbsp;</span><a href="javascript:void(0);" onclick="'+target+'(\'unflollow\',\''+target+'\','+uid+')">取消</a>';
	}else{
		html = '<a href="javascript:void(0);" onclick="'+target+'(\'dofollow\',\''+target+'\','+uid+')">加关注</a>';
	}
	return html;
}
...
ui = window.ui ||{
	...
	uppic:function(){
		ui.box.load(U('home/Account/uppic'), {title:'上传头像'});
	},	
...
getarea_ig:function(prefix,init_style,init_p,init_c,init_a){
		var style = (init_style)?'class="'+init_style+'"':'';
		var html = '<li class="s_li2"><select name="'+prefix+'_province" id="'+prefix+'_province" '+style+'><option>省/直辖市</option></select></li><li class="s_li2"> <select name="'+prefix+'_city" id="'+prefix+'_city" '+style+'><option value=0>不限</option></select></li><li class="s_li2"><select name="'+prefix+'_area" id="'+prefix+'_area" '+style+'><option value=0>不限</option></select></li>';
		document.write(html);
		// _PUBLIC_+'/js/area.js'
		...
		$.getJSON(U('home/Public/getArea'), function(json){
			...
			$.each(json,function(i,n){
				...
				if(n.id==init_p){
					 ...
					 $.each(n.citys,function(j,m){
							//for(var p in m){
								cselected = (p.id==init_c)?'selected="true"':'';
								 $.each(m.areas,function(k,o){
										for(var p in o){
											areaed = (p==init_a)?'selected="true"':'';
											area+='<option value="'+p+'" '+areaed+'>'+o[p]+'</option>';
										};
										$("select[name='"+prefix+"_area']").html(area);
								 });
								city+='<option value="'+m.id+'" '+cselected+' rel="'+j+'" >'+m.name+'</option>';
							//};
					 });
					 $("select[name='"+prefix+"_city']").html(city);
				}
			...
			$("select[name='"+prefix+"_province']").live('change',function(){
				...
				if( handle ){
					...
					$.each(t,function(j,m){
						//for(var p in m){
							city+='<option value='+m.id+' rel="'+j+'" >'+m.name+'</option>';
						//};
					});
				};
				$("select[name='"+prefix+"_city']").html(city);
			...
			$("select[name='"+prefix+"_city']").live('change',function(){
				var area='<option>不限</option>';
				var handle1 =  $("select[name='"+prefix+"_province']").find('option:selected').attr('rel');
				var handle =  $(this).find('option:selected').attr('rel');
				if( handle ){
					var t =  json[handle1].citys[handle].areas;
					$.each(t,function(j,m){
						for(var p in m){
