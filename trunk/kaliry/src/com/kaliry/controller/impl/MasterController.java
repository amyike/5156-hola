...
public class MasterController extends BaseController {
	...
	public String edit(HttpServletRequest request, Master master) {
		...
		try {
			...
			// 判断省份是否修改
			String province = master.getProvince();
			if (StringUtils.isNotBlank(province)
					&& !province.equals(_master.getProvince())) {
				_master.setProvince(province);
			}
			// 判断省份是否修改
			String city = master.getCity();
			if (StringUtils.isNotBlank(city) && !city.equals(_master.getCity())) {
				_master.setCity(city);
			}
			// 判断分公司是否修改
			String company = master.getCompany();
			if (StringUtils.isNotBlank(company)
					&& !company.equals(_master.getCompany())) {
				_master.setCompany(company);
			}
